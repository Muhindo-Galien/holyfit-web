 ...
 *
 * **It autoplays for everyone.** This used to hold the poster frame and offer a
 * play button to anyone whose system asks for reduced motion. That is the
 * conventional reading of the setting, and it is overridden here deliberately:
 * the loop is the only place on the page the app is shown *running*, and a
 * visitor who lands on a still of it has been shown a screenshot they already
 * have four of further down. Autoplay is the product demo, not decoration.
 *
 * No `controls`, for the same reason there were none before on the autoplaying
 * path: the native control bar renders *inside* the handset, and a scrub bar
 * across the bottom of the screen is the one thing that stops a mockup reading
 * as a phone. Every visitor now gets exactly what a visitor without the
 * preference set already got — the exception is gone, nothing else changed.
 *
 * Nothing here needs the client any more — the preference query was the only
 * reason this file carried `'use client'`, and it no longer does.
 */
export function DeviceVideo({ src, poster, label }: { src: string; poster: string; label: string }) {
  return (
    <Device>
      <video
        className="h-full w-full object-cover"
